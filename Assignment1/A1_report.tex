...
The following step sizes were tested to determine which step size yielded the best results for the validation set:  \(10^{-6}, 5\times10^{-6}, 10^{-5}, 5\times10^{-5}, 10^{-4}, 5\times10^{-4}, 10^{-3}, 5\times10^{-3}, 10^{-2}, 5\times10^{-2}\). Based on these values, the best \(\alpha\) was found to be \(5\times10^{-6}\). 
...
The mean is a good filler for missing values if that particular attribute is continuous numerical. The mode may be a better choice for categorical/ordinal data, since the mean may produce a decimal value that does not fit any of the categories. \\
Since most of the features of this dataset are numeric decimal (i.e. continuous), the mean of each feature is a good choice to fill in missing entries for that particular feature.
